...
class HomeViewController: UIViewController, ImageScannerControllerDelegate {
    ...
    lazy var selectPhotoButton: UIButton = {
        let button = UIButton()
        button.setImage(UIImage(named: "gallery", in: Bundle(for: SwiftDocumentScannerPlugin.self), compatibleWith: nil)?.withRenderingMode(.alwaysTemplate), for: .normal)
    ...
    @objc func selectPhoto() {
        if let window = UIApplication.shared.keyWindow {
            ...
            scanPhotoVC.scannerOptions = self.scannerOptions
            if #available(iOS 13.0, *) {
